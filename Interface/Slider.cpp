...
Slider::Slider(float x, float y, float width, float min, float max) : minValue(min), maxValue(max), isDragging(false) {
    ...
    thumb.setPosition(x + width / 2 - thumb.getRadius(), y - thumb.getRadius() + track.getSize().y / 2.0f);
}
...
void Slider::handleEvent(sf::Event event, sf::RenderWindow &window, sf::Music &music) {
    ...
    if (isDragging && event.type == sf::Event::MouseMoved) {
        ...
        if (trackWidth > 0) {
            ...
            if (usableWidth <= 0) usableWidth = 1.0f;
